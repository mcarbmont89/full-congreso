...
CREATE TABLE public.legislators (
    ...
    CONSTRAINT legislators_status_check CHECK (((status)::text = ANY ((ARRAY['Activo'::character varying, 'Inactivo'::character varying, 'Licencia'::character varying])::text[])))
);
...
CREATE TABLE public.radio_programs (
    ...
    CONSTRAINT radio_programs_status_check CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'inactive'::character varying, 'archived'::character varying])::text[])))
);
...
CREATE TABLE public.video_news (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT video_news_status_check CHECK (((status)::text = ANY ((ARRAY['draft'::character varying, 'published'::character varying, 'scheduled'::character varying, 'archived'::character varying])::text[])))
);
...
COPY public.news (id, title, summary, content, image_url, category, published_at, created_at, updated_at, status) FROM stdin;
\.
...
1	admin	$2b$10$EVvKjNTEZfWwr.ExasbRpexkzqemfO14JfDPUQ4el.BP4QqTMtRLW	admin	t	2025-07-18 06:40:39.012776	2025-07-18 06:40:39.012776
2	cmscanal	$2b$10$mTz.zmhXzndDOYKQHoYtQu25U5Z06WtPGCG1RevD9D8QGiuJcrDYW	admin	t	2025-07-18 06:40:39.012776	2025-07-18 06:40:39.012776
\.
...
SELECT pg_catalog.setval('public.news_id_seq', 28, true);
