    ...
    const fetchStreams = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Show streams that are live or in recess, hide offline streams
          ...
          if (streamId) {
            ...
            // Set the first live or recess stream as current
            const availableStream = onlineStreams.find((stream: LiveStream) => 
              stream.status === 'live' || stream.status === 'recess'
            )
            if (availableStream) {
        ...
        <section className="bg-purple-900 text-white py-8">
          <div className="container mx-auto px-4">
            ...
            <p className="text-xl max-w-3xl mx-auto text-center mb-8">
              Transmisiones en vivo de las sesiones del Congreso, comisiones y eventos especiales
            </p>
            ...
            <div className="relative aspect-video max-w-6xl mx-auto bg-black rounded-lg overflow-hidden mb-6">
              ...
              {/* Live/Recess indicator */}
              {currentStream && (currentStream.status === 'live' || currentStream.status === 'recess') && (
                <div className={`absolute top-4 left-4 text-white px-3 py-1 rounded-full text-sm font-bold ${
                  currentStream.status === 'live' ? 'bg-red-600' : 'bg-orange-600'
                }`}>
                  {currentStream.status === 'live' ? '● EN VIVO' : '● EN RECESO'}
                </div>
              )}
            ...
            <div className="text-center mt-4 mb-6">
              ...
              <p className="text-lg text-gray-200">
                {currentStream?.channel || "Transmisión principal"}
              </p>
            </div>
                    ...
                    <div className="relative h-48 w-full">
                      ...
                      {(stream.status === 'live' || stream.status === 'recess') && (
                        <div className={`absolute top-2 left-2 text-white px-2 py-1 rounded text-xs font-bold ${
                          stream.status === 'live' ? 'bg-red-600' : 'bg-orange-600'
                        }`}>
                          {stream.status === 'live' ? '● EN VIVO' : '● EN RECESO'}
                        </div>
                      )}
                      </div>
                    <div className="p-4">
