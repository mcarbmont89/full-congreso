...
export async function POST() {
  try {
    ...
    // First, check which episodes will be published
    const checkResult = await pool.query(`
      SELECT id, title, publish_date, published
      FROM radio_episodes 
      WHERE published = false AND publish_date <= NOW()::date
    `)
    
    console.log(`Found ${checkResult.rows.length} scheduled radio episodes ready to publish:`, 
    ...
    // Update scheduled radio episodes that should now be published
